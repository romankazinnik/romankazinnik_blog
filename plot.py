...
import glob
files = sys.argv[1:] or sorted(glob.glob("tracker*.csv"))     # default: every tracker*.csv in this directory
parts = [pd.read_csv(f) for f in files]
df = pd.concat(parts, ignore_index=True, join="inner")          # common columns only (older files lack e.g. `gpu`)
print("loaded " + ", ".join(f"{f} ({len(d)} rows)" for f, d in zip(files, parts)))
...
for ax, P in zip(axes[0], sizes):
    g = gpu[gpu["P"] == P]
    calm = g[g["u_pred"] < 0.5]                                    # a_eff is a server property; only trust it from unsaturated runs
    a_eff = (calm if len(calm) else g)["a_eff_ms"].median()
    F = g[g["seq_len"] == g["seq_len"].min()]["F_eff_tflops"].median() * 1e12   # F from the narrowest-S table (least noisy)
